...
function initThree() {
    ...
    const material = new THREE.MeshPhysicalMaterial({
        color: 0xF8F4EC,
        emissive: 0xEFEDE6,
        emissiveIntensity: 0.1, // Reduced to avoid bloom
        roughness: 0.7,
        metalness: 0.1,
        transmission: 0.0,
        thickness: 2.0,
...
function updateClock() {
    ...
    // Determine AM/PM and set background
    // Determine AM/PM
    ...
    // Update Hours - Light Green
    hourFronds.forEach((frond, index) => {
        const sphere = frond.userData.tipSphere;
        if (index < hours) {
            sphere.material.color.setHex(0xdce17a); // Light Green
            sphere.material.emissive.setHex(0xb8c960); // Darker Green Emissive
            sphere.material.emissiveIntensity = 1.0; // Moderate glow
            ...
            sphere.material.emissiveIntensity = 0.2; // Slightly brighter than dim
        }
    });

    // Update Minutes - Light Purple
    minuteFronds.forEach((frond, index) => {
        ...
        if (index < minutes) {
            ...
            sphere.material.color.setHex(0xd077a5); // Light Purple (Purplish-Pink)
            sphere.material.emissive.setHex(0xa85580); // Darker Purple Emissive
            sphere.material.emissiveIntensity = 1.2; // Balanced for visibility and glow
            ...
            sphere.material.emissiveMap = null;
            sphere.material.color.setHex(0xFFFFFF); // Pure White
            sphere.material.emissive.setHex(0xCCCCCC); // Light Grey Emissive for White
            sphere.material.emissiveIntensity = 0.2; // Slightly brighter than dim
        }
    });
